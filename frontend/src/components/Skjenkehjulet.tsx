...
const Skjenkehjulet: React.FC<SkjenkehjuletProps> = ({
  ...
  const startIdleWheelSpin = () => {
    ...
    const drawFrame = () => {
      ...
      wheelCategories.forEach((category, index) => {
        const startAngle = idleRotationRef.current + index * segmentAngle;
      ...
      idleRotationRef.current += 0.02;
      idleAnimRef.current = requestAnimationFrame(drawFrame);
    };
  ...
  const startWheelAnimation = (targetIndex: number) => {
    ...
    const animate = () => {
      ...
      currentSpeed *= 0.97; // Slow down more noticeably
