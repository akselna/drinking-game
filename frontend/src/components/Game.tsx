...
const Game: React.FC = () => {
  ...
  // Update the renderGame function to include Skjenkehjulet case
  const renderGame = () => {
    if (isReconnecting) {
      return (
    ...
    switch (sessionData.gameType) {
      ...
      case GAME_TYPES.SKJENKEHJULET: // Add new case for Skjenkehjulet
        return (
          <Skjenkehjulet
            sessionId={sessionData.sessionId}
            players={sessionData.players}
  ...
  const isSkjenkehjulet = sessionData.gameType === GAME_TYPES.SKJENKEHJULET;

  return (
    <div className={`game-container${isSkjenkehjulet ? " full-width" : ""}`}>
      {/* Show the participant panel unless Skjenkehjulet is active */}
      {!isSkjenkehjulet &&
        !isReconnecting &&
        !error &&
